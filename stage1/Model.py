...
class PSPDec(nn.Module):
    ...
    def __init__(self, nIn, nOut, downSize, upSize=48):
        ...
        self.features = nn.Sequential(
            nn.AdaptiveAvgPool2d(self.downSize),
            # NOTE: we trained our network at fixed size. If you want to train the network at variable size,
            # use the below version.
            nn.Conv2d(self.nIn, self.nOut, 1, bias=False),
            nn.BatchNorm2d(self.nOut, momentum=0.95, eps=1e-3),
            nn.ReLU(True),
    ...
    def forward(self, x):
        b, c, w, h = x.size()
        out = self.features(x)
        out = F.upsample(out, (w, h), mode='bilinear')
        return out
...
class ResNetC1(nn.Module):
    ...
    def __init__(self, classes):
        ...
        self.class_0 = nn.Sequential(
            nn.Conv2d(16 + 5 * classes, 5*classes, 3, padding=1, bias=False),
            nn.BatchNorm2d(5*classes, momentum=0.95, eps=1e-3),
            nn.ReLU(True),
            nn.Conv2d(5*classes, 2, 7, padding=3, bias=False)
        )
        ...
        self.p10 = PSPDec(classes + 256, 64, 80, 96)
        self.p20 = PSPDec(classes + 256, 64, 64, 96)
        self.p30 = PSPDec(classes + 256, 64, 48, 96)
        self.p40 = PSPDec(classes + 256, 64, 36, 96)

        self.class_1 = nn.Sequential(
            nn.Conv2d(classes + 256 + 64 * 4, 256, 3, padding=1, bias=False),
            ...
            nn.Conv2d(64, classes, 1, bias=False),
        ...
        self.p1 = PSPDec(512, 128, 40, 48)
        self.p2 = PSPDec(512, 128, 32, 48)
        self.p3 = PSPDec(512, 128, 24, 48)
        self.p4 = PSPDec(512, 128, 18, 48)
    ...
    def forward(self, input1):
        ...
        # p4 = self.p4(output3)
        #  PSPDEC
        psp1 = torch.cat([output3, self.p1(output3), self.p2(output3), self.p3(output3), self.p4(output3)], 1)
        output3 = self.classifier(psp1)

        output3 = self.upsample_3(output3)

        combine_up_23 = torch.cat([output3, output1], 1)
        #  PSPDEC
        psp2 = torch.cat([combine_up_23, self.p10(combine_up_23), self.p20(combine_up_23), self.p30(combine_up_23), self.p40(combine_up_23)], 1)
        output23_hook = self.class_1(psp2)

        # up = self.upsample_3(output23_hook)
        output23_hook = self.upsample_2(output23_hook)
        combine_up = torch.cat([output0, output23_hook], 1)
        #PSPDEC
        psp3 = torch.cat([combine_up, self.p01(combine_up), self.p02(combine_up), self.p03(combine_up), self.p04(combine_up)], 1)
        output0_hook = self.class_0(psp3)

        classifier = self.upsample_1(output0_hook)

        # classifier = F.sigmoid(classifier)
...
class ResNetD1(nn.Module):
    ...
    def __init__(self, classes):
        ...
        # self.p10 = PSPDec(256, 64, 80, 96)
        # self.p20 = PSPDec(256, 64, 64, 96)
        # self.p30 = PSPDec(256, 64, 48, 96)
        # self.p40 = PSPDec(256, 64, 36, 96)
